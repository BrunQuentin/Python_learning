...
# on importe les donnée du fichier donnees.py
from donnees import *
...
def recup_scores():
    ...
    On s'appuie sur ichier_scores défini dans donnees.py"""
...
def save_scores(scores):
    ...
    fichier_scores.close()
# Fonctions gérant les lettres saisis par le joueur
...
def nom_joueur():
    ...
    nom_joueur = input("tapper votre nom d'utilisateur: ")
    nom_joueur = nom_joueur.capitalize()
    if len(nom_joueur)<4 or not nom_joueur.isalnum(): 
        # isalnum = is alphanumeric 
        
        print("votre nom de joueur est incorrect.")
        # on appelle la fonction pour redemander un autre
        # nom
        return nom_joueur()
    else:
        return nom_joueur
...
def recup_lettre():
    ...
    lettre = input("tapper une lettre: ")
    lettre = lettre.lower()
